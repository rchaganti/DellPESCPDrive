...
$referenceTag = Connect-PEServer -DRACIPAddress $referenceSystemDRAC -DRACCredential $DRACCredential

#Add the DRAC as containers
$differenceTags = @()
foreach ($server in $differenceSystemDRAC)
{
...
$componentsToValidate = @('iDRAC.Embedded.1','System.Embedded.1','BIOS.Setup.1-1')
